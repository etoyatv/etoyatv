...
async function initDb() {
  ...
  try {
    ...
    try { await connection.query(`ALTER TABLE ip_bans MODIFY COLUMN ban_type ENUM('registration', 'all', 'account') DEFAULT 'registration'`); } catch (e) { }
    await connection.query(`
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS ip_bans (
        ...
        ban_type ENUM('registration', 'all') DEFAULT 'registration',
        reason TEXT,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS album_records (
        ...
        FOREIGN KEY (record_id) REFERENCES records(id) ON DELETE CASCADE,
        UNIQUE KEY unique_album_record (album_id, record_id)
      )
    `);
